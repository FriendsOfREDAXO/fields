(function() {
    ...
    /**
     * Fields Structure Logic
     * Handles wrapping of YForm elements into Grid/Flex Layouts
     */

    function initFieldsStructures(scope) {
        scope = scope || document;
        
        var markers = scope.querySelectorAll('.fields-structure-marker:not(.js-processed)');
        ...
        Array.prototype.forEach.call(markers, function(marker) {
            ...
            if (type === 'start') {
                ...
                // Find content until next structure marker (start or end)
                // We use a simplified selector here, assuming no deep nesting of SECTIONS within SECTIONS directly without markers
                var content = getNextSiblingsUntil(marker, '.fields-structure-marker');
                
                if (content.length > 0) {
                    var wrapper = document.createElement('div');
                    wrapper.className = 'fields-structure-grid';
                    // Apply styles
